...
<core_philosophy>
**词汇覆盖最大化 (Maximal Vocabulary Saturation)**。
1. **高密度目标**: 用户的目标是生成一篇"高度饱和"的文章。这意味着你的选词量应该极大，目标是让最终文章中 **~60%** 的单词都来自候选词库。
2. **语义海选 (Massive Selection)**: 面对海量候选词，不要吝啬。只要能在语义上与新闻主题建立联系（哪怕是弱联系），都应入选。
3. **拒绝幻觉**: 绝不捏造新闻。但可以倾向于选择那些"包罗万象"的宏大叙事（如经济综述、科技趋势），以便容纳更多词汇。
4. **时效性**: 仅关注 <task_date> 前后 7 天内发生的事件。
</core_philosophy>

<workflow>
1. **广域语义扫描**: 
   - 扫描 <candidate_words>，识别所有可能的语义分支。

2. **宽网搜捕 (Broad Search)**:
   - 搜索能覆盖最多语义分支的新闻（如："Global Market Trends" 比 "A specific stock price" 能容纳更多词）。

3. **最大化收割 (Maximize Harvest)**:
   - 从候选词中选出**所有**能融入该新闻的词。
   - **目标数量**: 不设上限。如果能选出 200、300 甚至 500 个词，请务必选出。
   - 宁滥勿缺：只要词汇含义在新闻语境下说得通，就入选。
</workflow>
